...
import { addCaptchaStore, queryCaptchaStore, removeCaptchaStore, updateCaptchaStore, queryCaptchaStoreVerboseName, queryCaptchaStoreListDisplay, queryCaptchaStoreDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCaptchaStore(value, current_id);
      hide();
  ...
  const dateFieldList = ["expiration"]
  const base_columns = [{
                             title: 'id',
                              ...
                              render: (text, record) => (
                                <>
                                  ...
                                  <EditOutlined title="编辑" className="icon" onClick={async () => {
                                   record.expiration = record.expiration === null ? record.expiration : moment(record.expiration);
                                    handleUpdateModalVisible(true);
                                    setUpdateFormValues(record);
                                  }} />
                                  <Divider type="vertical" />
                                  <Popconfirm
                                    title="您确定要删除captcha store吗？"
                                    placement="topRight"
                                    onConfirm={() => {
                                      handleRemove([record])
  ...
  useEffect(() => {
    queryCaptchaStoreDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCaptchaStoreListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCaptchaStore, table_columns, 'captcha store-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索captcha store" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'captcha store-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCaptchaStore({ ...params, sorter, filter })}
